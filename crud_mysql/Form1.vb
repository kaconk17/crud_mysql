...
Public Class Form1
    ...
    Sub cek_id()
        conn.Open()
        cmd = New MySqlCommand("SELECT * FROM biodata where nis = '" & txt_nis.Text & "'", conn)
        ...
        Select Case MsgBox("Apakah anda akan menyimpan data ini ?", MsgBoxStyle.YesNo)
            Case MsgBoxResult.Yes
                ...
                Try
                    ...
                    cmd = New MySqlCommand("INSERT INTO biodata (nis, nama, nisn, jeniskelamin, tempatlahir, tanggallahir, agama, alamat, sekolahasal) values('" & txt_nis.Text & "','" & txt_nama.Text & "', '" & txt_nisn.Text & "', '" & ComboBox_kelamin.Text & "', '" & txt_tempatlahir.Text & "', @tanggal, '" & txt_agama.Text & "', '" & txt_alamat.Text & "', '" & txt_sekolah.Text & "')", conn)
                    cmd.Parameters.Add("@tanggal", MySqlDbType.Date).Value = DateTimePicker1.Value
                    cmd.ExecuteNonQuery()
                    conn.Close()
        ...
        Select Case MsgBox("Apakah anda akan mengupdate data ini ?", MsgBoxStyle.YesNo)
            Case MsgBoxResult.Yes
                Try
                    ...
                    cmd = New MySqlCommand("UPDATE biodata SET nis='" & txt_nis.Text & "' , nama ='" & txt_nama.Text & "' , nisn='" & txt_nisn.Text & "' , jeniskelamin = '" & ComboBox_kelamin.Text & "', tempatlahir='" & txt_tempatlahir.Text & "' , tanggallahir = @tanggal, agama ='" & txt_agama.Text & "' , alamat = '" & txt_alamat.Text & "', sekolahasal ='" & txt_sekolah.Text & "' WHERE id_siswa = '" & txt_id.Text & "' ", conn)
                    cmd.Parameters.Add("@tanggal", MySqlDbType.Date).Value = DateTimePicker1.Value
                    cmd.ExecuteNonQuery()
                    conn.Close()
    ...
    Private Sub btn_hapus_Click(sender As Object, e As EventArgs) Handles btn_hapus.Click
        ...
        Select Case MsgBox("Apakah anda akan menghapus data nomer " & id & " ?", MsgBoxStyle.YesNo)
            Case MsgBoxResult.Yes
                cmd = New MySqlCommand("DELETE FROM biodata WHERE id_siswa = '" & id & "'", conn)
                conn.Open()
                cmd.ExecuteNonQuery()
                conn.Close()
